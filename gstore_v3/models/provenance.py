...
class ProvBase(Base):
    """provenance base data (data series)
    ...
    Note:
        ...
        date_added/_modified should be set through the api (files on disk, no row-based trigger in place)
...
class ProvOntology(Base):
    """supported ontologies for the app

    Note:
        must be specific to the base file, i.e. "elseweb" is not enough to id the modis structure
...
class ProvMapping(Base):
    """supported mappings between the base standard and the ontologies
    ...
    Note:
        ...
        ontology_id + inputstandards_id+output_format must be unique
...
class ProvInputStandard(Base):
    ...
    Note:
        define the supported inputs with the expected format (iso ds as xml)
