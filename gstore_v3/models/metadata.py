...
'''
XSLT input and output mappings
these are located in the xslt_path config location 
in an xslt subdir
'''
TO_GSTORE_XSLTS = {
...
class OriginalMetadata(Base):
    """documentation xml as provided to gstore
...
class DatasetMetadata(Base):
    """the GSTORE schema preferred documentation
    ...
    def transform(self, out_standard, out_format, xslt_path, metadata_info, validate=True):
        """trasnform gstore schema xml to supported standard

        Notes:
            some elements are updated based on info in the database
            like the description/title, online linkages, distribution
            links, etc.
        ...
        if out_standard == 'GSTORE':
            #TODO: add the xml schema location (to the static gstore.xsd location)
            return updated_xml
        ...
        if validate:
            ...
            if 'ERROR' in valid:
                #TODO: change to a better invalid error
                return None
...
class CollectionMetadata(Base):
    """gstore schema documentation for collections

    Note:
        can only be represented as iso ds now
...
class MetadataStandards(Base):
    """list of supported metadata stanadrds and the supported formats
